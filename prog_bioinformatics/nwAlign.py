...
def get_alignment_seqs()-> list:
    ...
    :return: list of sequence 1 bases, list of sequence 2 bases
    """
...
def generate_scoring_matrix(base_matrx: list, row: int, col: int, seq_1: list, seq_2: list):
    ...
    if 1 in [row, col]:
        base_matrx[row][col] = [0 if row == col else base_matrx[row][col - 1]+score['gap'] if row == 1 else base_matrx[row - 1][col]+score['gap']][0]
    else:
        base_matrx[row][col] = [max(base_matrx[row][col - 1]+score['gap'], base_matrx[row - 1][col - 1]+score['match'], base_matrx[row - 1][col]+score['gap']) if seq_1[row] == seq_2[col] else max(base_matrx[row][col - 1]+score['gap'],base_matrx[row - 1][col - 1]+score['mismatch'],base_matrx[row - 1][col]+score['gap'])][0]
...
def display_results(results: dict):
    ...
    print("Alignment score: ",sum([1 if x == '|' else -1 for x in symbol]))
    return
...
if __name__ == '__main__':
    ...
    result = compute_backtracking(matrx=score_matrx, row=len(x)-1, col=len(y)-1, result={'alignment': []}, seq_1=x, seq_2=y)
